...
export class AppShare {
  ...
  wxShare(scene,title,des,thumb,link) {
    var loading = this.loadingCtrl.create({ showBackdrop: false });
    loading.present();
    try {
      Wechat.share({
        message: {
          title: title? title:"菜谱大全",
          description: des?des:"菜谱大全介绍",
          thumb: thumb? thumb:this.image,
          mediaTagName: "TEST-TAG-001",
          messageExt: "",  // 这是第三方带的测试字段
          messageAction: "", // <action>dotalist</action>
          media: {
            type: Wechat.Type.WEBPAGE,
            webpageUrl: link?link:this.link
          }
        },
  ...
  qqShare(scene,link,title,des,img) {
    var loading = this.loadingCtrl.create({ showBackdrop: false });
    loading.present();
    try {
      var args: any = {};
      ...
      args.url = link?link:this.link;
      args.title = title?title:"菜谱大全";
      args.description = des?des:"简单介绍";
      args.image = img?img:this.image;
